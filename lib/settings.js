 ...
 * 用户文档写在上层；`installYoloSettings` 走 `SettingsProvider.installSection`
 * 的可选-settings 消费者接线，部署无 settings 服务时打 debug 日志跳过（零侵入）。
 *
 * DSH 0.1.2-alpha.4 兼容：dsh-settings 改为类服务——默认导出 `SettingsProvider`，
 * `installSettingsSection` / `settingsNamespace` 函数导出已删除；命名空间改为
 * 纯 kebab-case 字符串字面量（模板校验 `^[a-z][a-z0-9-]*$`），消费接线改为
