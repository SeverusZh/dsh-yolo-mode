 ...
 *
 * v0.3.0 重做（design.md §12.3）；v0.5.2 迁移至 0.1.7 的 SettingsForms：
 *   - `inject: ['llm','settings']`，用 `ctx.llm` / `ctx.settings`；
 *   - 设置声明为本插件的 Config schema（`export const Config`，volatile 字段可热更），
 *     `effectiveConfig()` 每次裁决用 `config.<field>.get()` 读取实时值后 `normalizeConfig`；
 *   - 页面策略经 `installYoloSettingsPage`（`settings.configure({auto:false})`）注册；
...
import { YoloSettingsSchema, installYoloSettingsPage, readYoloConfig, pruneEmpty } from './settings.js';
...
/**
 * 插件 Config schema（0.1.7 起即权威设置 schema；条目 id `yolo-mode` 即
 * `describe()`/`mutate()` 的 ns）。cordis 从本导出读取并用其解析条目 config。
 */
export const Config = YoloSettingsSchema;
...
export function apply(ctx, config) {
  ...
  /** 每次裁决读取的有效配置（Config schema 默认 + 条目 config 实时值）后规范化。 */
  function effectiveConfig() {
    // 解析值含 schema 空集合默认（modes:[]/levels:{}），必须先剔除再 normalizeConfig，
    // 否则 fail-loud 拒绝空 modes（与 settings.js 的写时校验同一根因）。
    return normalizeConfig(pruneEmpty(readYoloConfig(config)));
  }

  // 初始配置 fail-loud：非法配置立即抛错 → 加载失败（与旧 apply 行为一致）。
  effectiveConfig();

  // ---- settings 页面策略（0.1.7 SettingsForms；auto:false = 自建 UI）----
  installYoloSettingsPage(ctx);
