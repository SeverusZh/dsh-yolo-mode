 ...
 *
 * DSH 0.1.2-alpha.4 兼容：dsh-settings 的 `settingsNamespace()` 品牌函数已
 * 删除，命名空间为纯 kebab-case 字符串字面量（`yolo-mode`）；`mutate` /
 * `describe` / `writable` 仍为 `SettingsProvider` 实例方法，
 * `SettingsConflictError` 导出保留（code 'SETTINGS_CONFLICT'）。
 *
 ...
 * alpha.4 起命名空间为纯 kebab-case 字符串（`settingsNamespace()` 品牌已删除），
 * `ns` 原样透传给 SettingsProvider.mutate（其内部按 `^[a-z][a-z0-9-]*$` 校验）。
