 ...
 *
 * DSH 0.1.7-rc.1 兼容：dsh-settings 默认导出由 `SettingsProvider` 改为
 * `SettingsForms`，但其 `describe` / `writable` / `mutate` / `replace` /
 * `update` 方法面与 `SettingsConflictError`（code 'SETTINGS_CONFLICT'）不变，
 * 故本桥的线协议与端点形状无需改动，仅内部实现改由 `SettingsForms` 承担。
 * 命名空间仍为纯 kebab-case 字符串字面量（`yolo-mode`，等于插件条目 id）。
 *
 ...
 * 0.1.7 起 `ns` 即插件条目 id，原样透传给 SettingsForms.mutate；非 volatile 路径
 * 会被其拒绝（映射为 settings-rejected）。
