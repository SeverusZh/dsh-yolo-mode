 ...
 *   - 非 POST → 404；content-type 非 json → 415；Host 非 loopback → 403；
 *   - body 非法 JSON → 400；信封非法 → 200 bad-request（固定 rpcId 'invalid-request'）；
 *   - method 与端点不符 → 200 bad-request；dispatch 抛错 → 500（headersSent 则 destroy）。
 ...
 *
 * DSH 0.1.2-alpha.4 兼容：dsh-settings 的 `settingsNamespace()` 品牌函数已删除，
 * 命名空间为纯 kebab-case 字符串字面量（`yolo-mode`）；`mutate` / `describe` /
 * `writable` 仍为 `SettingsProvider` 实例方法，`SettingsConflictError` 导出保留。
 */
 ...
 * alpha.4 起命名空间为纯 kebab-case 字符串，`ns` 原样透传给 SettingsProvider.mutate。
...
export async function handleYoloBridgeRequest(settings, getStatusPayload, req, res, openFile) {
  ...
  // 非 POST → 404（非字符串 method 必然 !== 'POST'，一并覆盖）。
  if (req.method !== 'POST') {
