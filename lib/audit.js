 ...
 * （openLogFile 端点）必须落到同一个文件。规则（design.md §8/§11.1）：
 * 配置显式给出非空 `auditFile` → 用配置值；否则回落默认 `%TEMP%/dsh-yolo/judge.log`。
 *
 * `openFileWithDefaultApp` 用 OS 默认应用打开文件（fire-and-forget）：
 * darwin → `open`；win32 → `cmd /c start "" "<file>"`；其余 → `xdg-open`。
 * 启动失败经 child 'error' 事件返回 { ok:false }，绝不抛出。
 *
 * 纯函数层（resolveAuditFile / auditFileExists / openerCommandFor）无副作用，可单测。
 */
...
/**
 * 解析生效的审计日志路径：显式非空 `auditFile` 优先（返回值去除首尾空白），否则回落默认。
