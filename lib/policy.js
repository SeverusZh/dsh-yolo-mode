 ...
 * 目标模式 → 策略：off → 全部 delegate；strict → ws-write=judge, dfa=delegate；
 ...
 *   strict → error='deny'、unsure='delegate'；permissive → unsure='allow'；
 *   off/balanced/yolo → 未声明回退行（judgeFallback 缺省 delegate）；custom → 空对象。
 */
...
/** 读取裁判分级值（levels.error / levels.unsure 之类）；未配置返回 undefined。 */
...
/**
 * 裁判失败/不确定时的回退决策（design.md §4，返回值域 `'allowed-once'|'rejected'|'delegate'`）：
 *   strict + error → 'rejected'；permissive + unsure → 'allowed-once'；
 *   custom → levels.error / levels.unsure：allow→'allowed-once'、deny→'rejected'、其余/缺省→'delegate'；
 *   其他任何组合 → 'delegate'。调用方将 'delegate' 视为 next()（转人工）。
