 ...
 * 配置合并（design.md §11.2）。
 * 语义：
 *   - 顶层字段浅合并：overlay 中非 undefined 字段覆盖 base（标量/数组/对象整值替换）；
 *   - judge 子对象按字段合并（仅覆盖 overlay 中非 undefined 的 judge 字段）；
 *   - modes / levels 若 overlay 提供（非 undefined）则整体替换。
 * 本函数不做校验与默认补全（交给 normalizeConfig 的 fail-loud）。
 * @param {object|undefined} base  基底配置（如插件行 config）
 * @param {...(object|undefined)} overlays 覆盖层（如 settings 分区 / HTTP body）
 * @returns {object} 合并结果（浅拷贝，未冻结）
 */
export function mergeConfig(base, ...overlays) {
  const result = {};
  ...
  for (const overlay of overlays) {
    ...
    for (const key of Object.keys(overlay)) {
      ...
      if (value === undefined) continue; // 未提供 → 保留底值
      if (key === 'judge' && isPlainObject(value)) {
        // judge 按字段合并，不整体替换。
        const merged = isPlainObject(result.judge) ? { ...result.judge } : {};
        for (const jk of Object.keys(value)) {
          if (value[jk] !== undefined) merged[jk] = value[jk];
        }
        result.judge = merged;
      } else {
        // 其余字段（含 modes / levels）整体替换。
        result[key] = value;
      }
    }
  }
