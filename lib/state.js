 ...
 *
 * 主插件条目（lib/index.js）在裁决时调用 `recordDecision(entry)` 写入审计统计与
 * 最近决策环形缓冲；桥接条目（lib/bridge-entry.js）经 `getStatusPayload(cfg)` 读取。
 * 二者为同一包内的模块单例，天然共享（design.md §12.3）。
 *
 ...
 * 选中预设时预填充（来自 lib/judge.js 的 defaultJudgePromptFor 与 lib/policy.js 的
 * defaultLevelsFor；二者均为无副作用纯函数）。
 */
...
/** 六预设的默认 { systemPrompt, levels } 映射（design.md §2/§13.1；statusView 携带，设置页预填充）。 */
